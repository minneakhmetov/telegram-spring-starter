...
class BotMethodsHolder {
    ...
    static class BotMethods {
        ...
        void addCallbackHandler(HandlerInvocation invocation) throws BotInitializingException {
            String handlerName = invocation.getMethod().getName();
            if (!callbackQueryHandlers.containsKey(handlerName)) {
                callbackQueryHandlers.put(handlerName, invocation);
            } else throw new BotInitializingException(String.format("CallbackQuery Handler %s is duplicated in methods [%s, %s]", handlerName,
        ...
        void addMessageHandler(HandlerInvocation invocation) throws BotInitializingException {
            Pattern pattern = Pattern.compile(invocation.getMethod().getName());
            if (!messageHandlers.containsKey(pattern)) {
                messageHandlers.put(pattern, invocation);
            } else throw new BotInitializingException(String.format("CallbackQuery Handler %s is duplicated in methods [%s, %s]", pattern,
                    invocation.getMethod().getDeclaringClass() + "#" + invocation.getMethod().getName(),
    ...
    BotMethods createBotMethods(String botName) {
        ...
        return botMethodsMap.put(botName, botMethods);
    }
